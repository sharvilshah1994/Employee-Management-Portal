...
$sql = "UPDATE `ledger_audit` SET `srno`='$srno',`date_checking`='$date_checking',`remarks`='$remarks',"
        . "`amount`='$amount',`particulars`='$particulars',`status`='$status',`voucher_no`='$voucher_no',`account_head`='$account_head',"
        . "`group`='$group' WHERE `parent_id`='$parent_id' AND ID='$id'";

if ($conn->query($sql) === TRUE) {
    
    echo "<script type='text/javascript'> window.location = 'http://$servername/dgsm1/portal/ledger_audit.php?id=$id&parent_id=$parent_id' </script>";
